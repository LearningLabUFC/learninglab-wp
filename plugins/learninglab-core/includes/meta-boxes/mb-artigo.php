...
/* ---------------------------------------------------------------
 * Meta Box: Informações Adicionais do Artigo (prêmio)
 * --------------------------------------------------------------- */
...
function salvar_meta_box_artigos( $post_id ) {
    if ( ! isset( $_POST['artigos_nonce'] ) || ! wp_verify_nonce( $_POST['artigos_nonce'], 'salvar_meta_box_artigos' ) ) return;
...
function mostrar_meta_box_autores_artigo( $post ) {
    ...
    ?>
    <style>
        #container-autores-wrapper { margin-top: 10px; }
        ...
        #lista-autores-selecionados li {
            ...
            background: #fff; margin: 0; cursor: move;
        }
        #lista-autores-selecionados li:last-child { border-bottom: none; }
        #lista-autores-selecionados li:hover { background: #f1f1f1; }
        .autor-handle { color: #ccc; margin-right: 8px; cursor: grab; }
        .autor-nome { flex-grow: 1; font-size: 13px; font-weight: 500; }
        .remove-autor { color: #d63638; cursor: pointer; padding: 2px; }
        .remove-autor:hover { color: #ff0000; }
        .controles-adicao { display: block; }
        #select-novos-autores { width: 100%; margin-bottom: 8px; box-sizing: border-box; }
        #btn-add-autor { width: 100%; text-align: center; margin-bottom: 10px; }
        .placeholder-msg { padding: 10px; color: #999; font-size: 12px; text-align: center; font-style: italic; }
    </style>
    ...
    jQuery(document).ready(function($){
        ...
        $('#btn-add-autor').click(function(e){
            ...
            if (!id) { alert('Por favor, selecione um membro na lista.'); return; }
            if (lista.find('input[value="'+id+'"]').length > 0) { alert('Este autor já está na lista.'); return; }
